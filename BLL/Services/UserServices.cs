...
namespace BLL.Services
{
    ...
    {
        //Generate Token
        public Tuple<bool, string> GenerateToken(long userId, string phone, string email, string roleBase)
        {
            try
            {
                DotNetEnv.Env.Load();
            ...
            try
            {
                using (StoreDbContext _context = new StoreDbContext())
                {
                    using (var _transaction = _context.Database.BeginTransaction())
                    {
                        ...
                        var role = _context.RoleBases.Where(r => r.Id == user.FkRoleBase).FirstOrDefault().Role;
                        var token = GenerateToken(user.Id, user.Phone, user.Email, role);
                ...
                using (var _context = new StoreDbContext())
                {
                    using (var _transaction = _context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            #region cek otp
                            var otpInDb = (from p in _context.OPTs.Where(o => o.FkUser == userId) orderby p.Id descending select p.Otp).FirstOrDefault();
                            if (otpInDb != data.otp)
                            {
                            ...
                            #region create token
                            var role = _context.RoleBases.Where(r => r.Id == user.FkRoleBase).FirstOrDefault().Role;
                            var token = GenerateToken(userId, user.Phone, user.Email, role);
                            if (token.Item1 == false)
                            {
        ...
        public Tuple<int, string> ValidateUserInput(User user)
        {
            try
            {
                using (var _context = new StoreDbContext())
                {
                    ...
                    #region cek phone
                    if (_context.Users.Any(p => p.Phone == user.Phone))
                    {
                        return Tuple.Create(-1, "Phone Already Exist");
                    }
                    #endregion cek phone

                    #region cek rolebase
                    if (!_context.RoleBases.Any(I => I.Id == user.FkRoleBase))
                    {
                        return Tuple.Create(-1, "Please Input valid role");
                    }
                    #endregion cek rolebase
